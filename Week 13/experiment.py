...
import numpy as np
import torch
from model import Decoder, CONFIG, allowed_attention, forward_pair
...
def evaluate(model,val,full=False):
    ...
    for s in val.manifest['sources']:
        ...
        for start,stop in groups:
            v,k=model.loss_sum(*batch(val,start,stop));summed+=float(v);n+=int(k)
...
def identity():
    ...
    return dict(model_sha256=sha(ROOT/'model.py'),runner_sha256=sha(ROOT/'experiment.py'),
...
def lr(consumed,budget):
    warmup=budget*0.02
    if consumed<warmup:return 6e-4*max(1e-3,consumed/warmup)
    p=min(1,(consumed-warmup)/(budget-warmup))
    return 6e-5+0.5*(6e-4-6e-5)*(1+math.cos(math.pi*p))


def benchmark(args):
    setup();folder=ROOT/'batch_search';folder.mkdir(exist_ok=True)
    result=dict(variant=args.variant,batch=args.batch,steps=args.steps,precision='float32',tf32=False,
    ...
    try:
        ...
        for i in range(args.steps):
            ...
            loss,n=m.loss_sum(ids,masks,seg,pos);(loss/n).backward()
...
def train(args):
    setup();folder=ROOT/args.name;folder.mkdir(exist_ok=True)
    ...
    cfg=dict(variant=args.variant,model=CONFIG,batch=args.batch,gradient_accumulation=1,
             training_tokens=args.tokens,precision='float32',tf32=False,seed=SEED,
             midpoint_h=0.5,euler_h=1.0,residual_dtype='float32',dropout=0,
             peak_lr=6e-4,min_lr=6e-5,warmup_token_fraction=0.02,weight_decay=0.1,
    ...
    emit(folder,'start',variant=args.variant,batch=args.batch,consumed=consumed,initial_probe=initial['loss'])
    ...
    while consumed<args.tokens:
        ...
        opt.zero_grad(set_to_none=True)
        rate=lr(consumed+available,args.tokens)
        for g in opt.param_groups:g['lr']=rate
        summed,n=m.loss_sum(*data)
    ...
    result=dict(state='COMPLETE',variant=args.variant,batch=args.batch,parameters=sum(p.numel() for p in m.parameters()),
                training_tokens=consumed,physical_tokens=physical,updates=updates,precision='float32',
...
def audit(args):
    setup();state=torch.load(ROOT/args.name/'final.pt',map_location='cpu',weights_only=True)
    ...
    losses=[]
    for m in (a,b):
        loss,n=m.loss_sum(*data);(loss/n).backward();losses.append(float(loss/n))
    ...
    with torch.no_grad():
        x=a.token_embedding(data[0])+a.position_embedding(data[3]);mask=allowed_attention(data[2])
