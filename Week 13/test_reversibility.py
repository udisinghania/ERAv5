...
import torch
from model import Decoder, CONFIG, ReversibleStack, allowed_attention, forward_pair
...
def compare(variant,device='cpu',amp=False,config=None,state=None,double=True):
    ...
    assert inversion<(0.02 if amp else 1e-4),(variant,inversion)
...
def main():
    ...
    (Path(__file__).parent/'correctness.json').write_text(json.dumps(report,indent=2)+'\n')
